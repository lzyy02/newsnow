...
APPOINT_SOURCE = "巨潮资讯-预约披露"
APPOINT_URL = "https://www.cninfo.com.cn/new/commonUrl?url=data/yypl"
CNINFO_SOURCE = "巨潮资讯-补充更正公告"
CNINFO_URL = "https://www.cninfo.com.cn/new/commonUrl/pageOfSearch?url=disclosure/list/search"
APPOINT_API = "https://www.cninfo.com.cn/new/information/getPrbookInfo"
HEADERS = {
...
def request_json(url: str, *, params: dict | None = None, data: dict | None = None, retries: int = 4, sleep: float = 0.5):
    last = None
    for attempt in range(1, retries + 1):
        try:
            response = SESSION.post(url, params=params, data=data, timeout=45)
...
def fetch_appointments(period: pd.Timestamp, sleep: float) -> pd.DataFrame:
    frames, errors = [], []
    for market in ("szsh", "bj"):
        params = {
            "sectionTime": period.strftime("%Y-%m-%d"),
            "firstTime": "",
            "lastTime": "",
            "market": market,
            "stockCode": "",
        ...
        try:
            payload = request_json(APPOINT_API, params=params, retries=4, sleep=max(sleep, 0.4))
            rows = payload.get("prbookinfos") or []
            if rows:
                normalized = []
                for row in rows:
                    values = list(row) if isinstance(row, (list, tuple)) else list(row.values())
                    values += [None] * max(0, 8 - len(values))
                    normalized.append(values[:8])
                frames.append(pd.DataFrame(normalized, columns=[
                    "股票代码", "股票简称", "预约披露日期", "实际公告日期时间",
                    "一次变更日期", "二次变更日期", "三次变更日期", "接口报告期",
                ]))
        except Exception as exc:
    ...
    for col in ["预约披露日期", "实际公告日期时间", "一次变更日期", "二次变更日期", "三次变更日期"]:
        ...
        for label, keyword in keywords:
            ...
            try:
                ...
                if x is not None and not x.empty:
                    ...
                    frames.append(x)
                log.append({"任务": "更正公告", "键": key, "状态": "成功", "行数": 0 if x is None else len(x), "错误": ""})
            except Exception as exc:
...
def main():
    ...
    result["数据下载状态"] = "已完成"
    result["口径说明"] = "实际公告日期来自巨潮预约披露主表，历史口径通常精确到日，不虚构时分秒；公告后首个可交易日统一取公告日期之后的第一个交易日。更正公告按标题中的报告年度与报告类型匹配。"
