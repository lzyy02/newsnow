...
def safe(step, fn, retries=2):
    ...
    for i in range(retries):
        try:
            ...
            LOGS.append([step, "成功", len(df), round(time.time()-t, 2), ""])
            return df
        except Exception as exc:
            err = f"{type(exc).__name__}: {exc}"
            time.sleep(i + 1)
    LOGS.append([step, "失败", 0, round(time.time()-t, 2), err])
    return pd.DataFrame()
...
def universe():
    frames = []
    df = safe("当前A股列表", ak.stock_info_a_code_name)
    if not df.empty:
        cc = next((c for c in df.columns if "代码" in c or "code" in c.lower()), df.columns[0])
        nc = next((c for c in df.columns if "简称" in c or "名称" in c or "name" in c.lower()), df.columns[1])
        x = df[[cc, nc]].copy(); x.columns = ["证券代码", "证券简称"]; x["证券来源"] = "当前A股"; frames.append(x)
    for fn_name, kwargs, label in [
        ("stock_info_sh_delist", {"symbol": "全部"}, "上交所退市"),
        ("stock_info_sz_delist", {"symbol": "终止上市公司"}, "深交所退市"),
    ]:
        if not hasattr(ak, fn_name):
            continue
        df = safe(label, lambda f=getattr(ak, fn_name), kw=kwargs: f(**kw))
        if df.empty:
            continue
    ...
    return out.dropna(subset=["证券代码"]).drop_duplicates("证券代码").sort_values("证券代码")


def run_shard():
    shard = int(os.environ.get("SHARD_INDEX", "0")); total = int(os.environ.get("SHARD_TOTAL", "8"))
    u = universe().reset_index(drop=True)
    u = u.iloc[[i for i in range(len(u)) if i % total == shard]]
    out = OUT / f"02A_公司股本变动_20150101_20260801_shard{shard:02d}.csv"
    failed = []
    ...
    def one(row):
        ...
        if df.empty:
            failed.append([code, name, source]); return df
        if "证券代码" not in df.columns: df.insert(0, "证券代码", code)
        if "证券简称" not in df.columns: df.insert(1, "证券简称", name)
        for c in ["公告日期", "变动日期"]:
            if c in df.columns:
                d = pd.to_datetime(df[c], errors="coerce")
                df = df[d.isna() | ((d >= pd.Timestamp(START)) & (d <= pd.Timestamp(END)))].copy()
                df[c] = pd.to_datetime(df[c], errors="coerce").dt.strftime("%Y-%m-%d")
        df["证券来源"] = source
        df["数据源"] = "巨潮资讯 p_stock2215 / AKShare stock_share_change_cninfo"
        df["口径说明"] = "已流通股份不等同于Choice严格自由流通股本；历史市值需与日行情合并"
        return df

    buf = []
    rows = list(u[["证券代码", "证券简称", "证券来源"]].itertuples(index=False, name=None))
    with ThreadPoolExecutor(max_workers=6) as pool:
        for fut in as_completed([pool.submit(one, r) for r in rows]):
            df = fut.result()
            if not df.empty: buf.append(df)
            if len(buf) >= 30:
                append_csv(out, pd.concat(buf, ignore_index=True, sort=False)); buf.clear()
...
def run_actions():
    ...
    for year in range(2015, 2027):
        for suffix, label in [("0630", "半年报"), ("1231", "年报")]:
            ...
            df = safe(f"分红送转-{period}", lambda p=period: ak.stock_fhps_em(date=p))
            if df.empty: continue
            df["报告期"] = period; df["报告类型"] = label; df["数据源"] = "东方财富/AKShare stock_fhps_em"
            for c in ["最新公告日期", "公告日期", "股权登记日", "除权除息日"]:
                if c in df.columns:
                    d = pd.to_datetime(df[c], errors="coerce")
                    df = df[d.isna() | (d <= pd.Timestamp(END))].copy(); df[c] = pd.to_datetime(df[c], errors="coerce").dt.strftime("%Y-%m-%d")
                    break
            frames.append(df)
    pd.concat(frames, ignore_index=True, sort=False).to_csv(OUT/"02B_公司行动_20150101_20260801.csv", index=False, encoding="utf-8-sig")
